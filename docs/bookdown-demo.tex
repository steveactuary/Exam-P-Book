...
\BeginKnitrBlock{example}[Hypergeometric Rocks]
\protect\hypertarget{exm:unnamed-chunk-22}{}{\label{exm:unnamed-chunk-22} \iffalse (Hypergeometric Rocks) \fi{} }We have \(15\) rocks in a bag. \(7\) rocks are red and \(8\) are black. We select \(5\) rocks. What is the probability of selecting exactly \(2\) red rocks and \(3\) black rocks.
...
\BeginKnitrBlock{exercise}[Hypergeometric Cards]
\protect\hypertarget{exr:unnamed-chunk-23}{}{\label{exr:unnamed-chunk-23} \iffalse (Hypergeometric Cards) \fi{} }We have a deck of \(40\) cards. \(30\) cards are red and \(10\) are black. We draw a hand of \(5\) cards. Show that the probability of drawing \(3\) black cards is:
...
\BeginKnitrBlock{definition}[Hypergeometric Distribution]
\protect\hypertarget{def:unnamed-chunk-24}{}{\label{def:unnamed-chunk-24} \iffalse (Hypergeometric Distribution) \fi{} }We randomly select \(n\) items from a population of \(N\) items. Let \(r\) represent the number of items from the population classified as a success, and \(k\) be the number of items in the selection classified as successes. Let \(X\) be the random variable representing the number of items in our selection considered successes.
...
\BeginKnitrBlock{example}[Memorization Is Bad]
\protect\hypertarget{exm:unnamed-chunk-25}{}{\label{exm:unnamed-chunk-25} \iffalse (Memorization Is Bad) \fi{} }We return to our example with \(15\) rocks in a bag. \(7\) rocks are red. We select \(5\) rocks. What is the probability of selecting exactly \(2\) red rocks. We abandon critical thought and use the formula.
...
\hypertarget{hypergeometric-vs.-binomial}{%
\subsection{Hypergeometric vs.~Binomial}\label{hypergeometric-vs.-binomial}}
...
The hypergeometric distribution is different than the binomial distribution because it samples \textbf{without replacement}. Let's change the problem and allow someone to win a prize multiple times. \(4\) names are drawn from a hat. Each time a name is drawn a prize is given and the name is put back in the hat (this is sampling \textbf{with replacement}). Since \(600\) of the \(1000\) people are actuaries, any time we make a selection to win a prize there is a probability of \(.6\) that the person is an actuary. We select \(4\) people, so the distribution of prizes given to actuaries is binomial with parameters \(n = 4\), \(p = .6\). In this example the probability of \(3\) actuaries winning prizes and \(1\) astronaut winning a prize is:
...
\BeginKnitrBlock{theorem}[Poisson Distribution]
\protect\hypertarget{thm:unnamed-chunk-27}{}{\label{thm:unnamed-chunk-27} \iffalse (Poisson Distribution) \fi{} }Let \(X\) have a Poisson distribution with parameter \(\lambda\). The probability of \(k\) occurences of something happening is:
...
\BeginKnitrBlock{example}[Poisson Scaling Rate]
\protect\hypertarget{exm:unnamed-chunk-28}{}{\label{exm:unnamed-chunk-28} \iffalse (Poisson Scaling Rate) \fi{} }Assume a Poisson model. The average rate of people spilling their coffee in the office throughout the work day is \(2.5\) spills per hour. What is the probability that \(2\) people spill their coffee between \(8\) A.M. and \(9\) A.M., and \(6\) people spill their coffee between \(11\) A.M. and \(1\) P.M.?
...
\BeginKnitrBlock{theorem}[Poisson Distribution]
\protect\hypertarget{thm:unnamed-chunk-29}{}{\label{thm:unnamed-chunk-29} \iffalse (Poisson Distribution) \fi{} }Let \(X\) have a Poisson distribution with parameter \(\lambda\).
\[E(X) = \lambda\]
\[V(X) = \lambda\]
...
Let \(X\) be the number of tries it takes to land a backflip if I land \(10%
\) of my backflips. This is an example of a geometric distribution. The geometric distribution is when you take independent bernoulli trials until the first success. Each backflip attempt is an independent bernoulli trial with the same probability of success (assuming I don't improve or get tired).
...
\protect\hypertarget{def:unnamed-chunk-30}{}{\label{def:unnamed-chunk-30} \iffalse (Geometric Distribution) \fi{} }We perform independent Bernoulli trials, each having a probability of success \(p\) and probability of failure \(q\). Let \(X\) represent the trial on which the first success occurs.
...
\BeginKnitrBlock{definition}[Geometric Distribution]
\protect\hypertarget{def:unnamed-chunk-31}{}{\label{def:unnamed-chunk-31} \iffalse (Geometric Distribution) \fi{} }Let \(X\) be a geometric random variable with parameter p.
...
\protect\hypertarget{def:unnamed-chunk-32}{}{\label{def:unnamed-chunk-32} \iffalse (Negative Binomial Distribution) \fi{} }Let \(Y\) be the number of failures until the \(rth\) success. We calculate the p.d.f. of \(Y\).
...
\BeginKnitrBlock{theorem}[Negative Binomial Distribution]
\protect\hypertarget{thm:unnamed-chunk-33}{}{\label{thm:unnamed-chunk-33} \iffalse (Negative Binomial Distribution) \fi{} }Let \(Y\) be the number of failures until the \(rth\) success.
\[E(Y) = \frac{r(1-p)}{p}\]
\[V(Y) = \frac{r(1-p)}{p^2}\]
\EndKnitrBlock{theorem}
...
\hypertarget{common-continuous-distributions}{%
\chapter{Common Continuous Distributions}\label{common-continuous-distributions}}

\hypertarget{bernoulli-distribution-1}{%
\section{Bernoulli Distribution}\label{bernoulli-distribution-1}}
